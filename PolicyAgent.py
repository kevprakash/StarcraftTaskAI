...
# python -m pysc2.bin.agent --map Simple64 --agent PolicyAgent.CGRUAgent --feature_screen_size 64,64 --difficulty easy
# RUNS VERY VERY SLOWLY
class CGRUAgent(base_agent.BaseAgent):
    def __init__(self):
        super().__init__()
        model = Model.CGRUPolicyModel
        self.actionModel = model([573], kernelSize=5, flatten=True)
        self.actionOptimizer = torch.optim.Adam(self.actionModel.parameters(), lr=10 ** (-3))
        self.argModels = [
            model([1], kernelSize=3, flatten=False),
            model([1], kernelSize=3, flatten=False),
            model([1], kernelSize=3, flatten=False),
            model([2], kernelSize=3, flatten=True),
            model([5], kernelSize=3, flatten=True),
            model([10], kernelSize=3, flatten=True),
            model([4], kernelSize=3, flatten=True),
            model([2], kernelSize=3, flatten=True),
            model([4], kernelSize=3, flatten=True),
            model([500], kernelSize=3, flatten=True),
            model([4], kernelSize=3, flatten=True),
            model([10], kernelSize=3, flatten=True),
            model([500], kernelSize=3, flatten=True)
        ]
        ...
        for m in self.argModels:
            self.argOptimizers.append(torch.optim.Adam(m.parameters(), lr=10 ** (-3)))
        self.screens = []
    ...
    def step(self, obs):
        ...
        self.screens.append(self.getNetworkInput(obs))
        networkInput = self.screens if len(self.screens) <= self.maxSequenceLength else self.screens[-self.maxSequenceLength:]
    ...
    def save(self):
        ...
        torch.save(actionState, 'CGRUAgent/ActionModel.pt')
        ...
        for i in range(len(self.argModels)):
            ...
            torch.save(argState, 'CGRUAgent/ArgModel' + str(i) + '.pt')

    def load(self):
        if os.path.exists('CGRUAgent/ActionModel.pt'):
            actionState = torch.load('CGRUAgent/ActionModel.pt')
            ...
            for i in range(len(self.argModels)):
                argState = torch.load('CGRUAgent/ArgModel' + str(i) + '.pt')
    ...
    def onNewEpisode(self):
        ...
        if len(self.scores) > 1:
            ...
            for screen in self.screens:
                ...
                modelLoss = (-valueDiff * torch.log(combinedPickChance)) + (1.0 * valueDiff**2) + (-0.001 * entropy)
